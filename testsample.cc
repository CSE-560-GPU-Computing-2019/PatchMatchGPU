...
float compareGridsEachPixel(const unsigned char *c_as_g_image, const unsigned char *g_image, const unsigned char *c_as_g_image_BASE, const unsigned char *g_image_BASE, int gridSizeX, int gridSizeY, int dataSizeX, int dataSizeY) {
    ...
    for (int row = -gridSizeY/2; row < gridSizeY/2; ++row) {
        for (int col = -gridSizeX/2; col < gridSizeX/2; ++col) {
...
void compareGridsEachPixelWithSamples(const unsigned char *g_image, const unsigned char *g_image_BASE, int gridSizeX, int gridSizeY, int dataSizeX, int dataSizeY) {
    ...
    for (int row = -gridSizeY/2; row <= gridSizeY/2; ++row) {
        for (int col = -gridSizeX/2; col <= gridSizeX/2; ++col) {
            ...
            g_Matrix[row + gridSizeY/2][col + gridSizeX/2] = g_image[col + row * dataSizeX];
        }
    }
...
void patchMatchEachPixelWithSamples(unsigned char *c_image, const unsigned char *c_as_g_image, const unsigned char *g_image, unsigned char *finalImage, int gridSizeX, int gridSizeY, int dataSizeX, int dataSizeY)
{
    // int widthIter = dataSizeX/gridSizeX;
    // int heightIter = dataSizeY/gridSizeY;
    int c_as_g_index_row = 0;
    ...
    // for (int i = 0; i < 200; ++i) {
        for (int row_g = 0; row_g < dataSizeY; ++row_g) { // Iterate over g_image
            ...
            for (int col_g = 0; col_g < dataSizeX; ++col_g) { // Iterate over g_image
                ...
                bestIndex = 0;
                // bestDiff = 0; 
                bestDiff = compareGridsEachPixel(c_as_g_image + samples[bestIndex].col + (samples[bestIndex].row * dataSizeX), 
                                                        g_image + g_index_col + (g_index_row * dataSizeX), 
                                                        c_as_g_image,
                                                        g_image,
                                                        gridSizeX, 
                ...
                for (int i = 1; i < NUMSAMPLE; ++i) {
                    absDiff = compareGridsEachPixel(c_as_g_image + samples[i].col + (samples[i].row * dataSizeX), 
                                                        g_image + g_index_col + (g_index_row * dataSizeX), 
                                                        c_as_g_image,
                                                        g_image,
                                                        gridSizeX, 
...
int main(int argc, char *argv[]){
    ...
    char grayscaleInputName[] = "forest2_grays.jpg"; // Image to be colored
    ...
    patchMatchEachPixelWithSamples(c_image, c_as_g_image, g_image, finalImage, maskCols, maskRows, c_width, c_height);
